...
## First Deployment
1. Register lifting-equipment engineers, plants, units, personnel and robots.
2. Import historical unit / load-test / design-rules records.
...
- human sign-off for `:high`/`:safety-critical` robot actions (e.g. final assembly on safety-critical units, load-test-certificate issuance)
...
## Operating states
intake : design-rules-verify : load-test-screen : approve : dispatch-unit : issue-load-test-certificate : audit
...
marking authority are the lifting-equipment manufacturer's own acts
