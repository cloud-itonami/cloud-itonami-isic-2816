...
(deftest audit-package-shape
  (let [db (seed-with-one-dispatch)
    ...
    (is (= "2816" (:isic pkg)))
    (is (= "cloud-itonami-isic-2816" (:business-id pkg)))
...
(deftest csv-bundle-has-headers-and-rows
  (let [db (seed-with-one-dispatch)
    ...
    (is (every? bundle ["units.csv" "ledger.csv" "dispatches.csv" "load-test-certificates.csv"]))
    (is (str/starts-with? (get bundle "units.csv") "id,unit-name,"))
    (is (re-find #"unit-1" (get bundle "units.csv")))
    (is (re-find #"JPN-LEQ-000000" (get bundle "dispatches.csv")))
