(ns liftingequip.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura 10t Overhead Bridge Crane OC-04" (:unit-name (store/unit s "unit-1"))))
      (is (= "JPN" (:jurisdiction (store/unit s "unit-1"))))
      (is (= 11.0 (:test-load-actual (store/unit s "unit-1"))))
      (is (= 10.0 (:test-load-min (store/unit s "unit-1"))))
      (is (= 12.5 (:test-load-max (store/unit s "unit-1"))))
      (is (false? (:load-test-defect-unresolved? (store/unit s "unit-1"))))
      (is (= 15.0 (:test-load-actual (store/unit s "unit-3"))))
      (is (true? (:load-test-defect-unresolved? (store/unit s "unit-4"))))
      (is (false? (:unit-dispatched? (store/unit s "unit-1"))))
      (is (false? (:load-test-certified? (store/unit s "unit-1"))))
      (is (= ["unit-1" "unit-2" "unit-3" "unit-4"]
             (mapv :id (store/all-units s))))
      (is (nil? (store/load-screen-of s "unit-1")))
    ...
    (testing label
      (testing "partial upsert merges, preserving untouched fields"
        (store/commit-record! s {:effect :unit/upsert
                                 :value {:id "unit-1" :unit-name "Sakura 10t Overhead Bridge Crane OC-04"}})
        (is (= "Sakura 10t Overhead Bridge Crane OC-04" (:unit-name (store/unit s "unit-1"))))
        (is (= 11.0 (:test-load-actual (store/unit s "unit-1"))) "unrelated field preserved"))
      (testing "verification / load-screen payloads commit and read back"
        ...
        (store/commit-record! s {:effect :load-test-screen/set :path ["unit-1"]
                                 :payload {:unit-id "unit-1" :verdict :resolved}})
        (is (= {:unit-id "unit-1" :verdict :resolved} (store/load-screen-of s "unit-1"))))
      (testing "unit dispatch drafts a record and advances the sequence"
        ...
        (is (= "JPN-LEQ-000000" (get (first (store/dispatch-history s)) "record_id")))
      ...
      (testing "load-test certificate drafts a record and advances the sequence"
        (store/commit-record! s {:effect :unit/mark-certified :path ["unit-1"]})
        (is (= "JPN-LTC-000000" (get (first (store/evidence-history s)) "record_id")))
        (is (= "load-test-certificate-draft" (get (first (store/evidence-history s)) "kind")))
        (is (true? (:load-test-certified? (store/unit s "unit-1"))))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (is (zero? (store/next-dispatch-sequence s "JPN")))
    (is (zero? (store/next-evidence-sequence s "JPN")))
    (store/with-units s {"x" {:id "x" :unit-name "n" :test-load-actual 11.0
                                   :test-load-min 10.0 :test-load-max 12.5
