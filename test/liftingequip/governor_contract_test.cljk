(ns liftingequip.governor-contract-test
  "The governor contract as executable tests -- the lifting-
  ...
  `casualty.governor-contract-test`. The single invariant under test:
    ...
    Lifting Equipment Advisor never dispatches a unit action or
    issues a load-test certificate the Lifting Equipment Governor
    would reject, `:actuation/dispatch-unit`/`:actuation/issue-
    load-test-certificate` NEVER auto-commit at any phase,
...
(defn- screen!
  "Walks `subject` through load-test-defect screening -> approve,
  ...
  `load-test-defect-is-held-and-unoverridable`)."
  [actor tid-prefix subject]
  (exec-op actor (str tid-prefix "-screen") {:op :load-test/screen :subject subject} operator)
...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Sakura 10t Overhead Bridge Crane OC-04" (:unit-name (store/unit db "unit-1"))) "SSoT actually updated")
...
(deftest unit-test-load-out-of-range-is-held
  (testing "a unit whose own proof-load test result falls outside its own spec bounds -> HOLD"
    (let [[db actor] (fresh)
          _ (verify! actor "t5pre" "unit-3")
          res (exec-op actor "t5" {:op :actuation/dispatch-unit :subject "unit-3"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:unit-test-load-out-of-range} (-> (store/ledger db) last :basis)))
      (is (empty? (store/dispatch-history db))))))

(deftest load-test-defect-is-held-and-unoverridable
  (testing "an unresolved load-test defect on a unit -> HOLD, and never reaches request-approval -- exercised via :load-test/screen DIRECTLY, not via the actuation op against an unscreened unit (see this actor's governor ns docstring / parksafety's ADR-2607071922 Decision 5 / turbine's and automotive's and machinetool's and pressureequip's ADR-0001s)"
    (let [[db actor] (fresh)
          res (exec-op actor "t6" {:op :load-test/screen :subject "unit-4"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:load-test-defect-unresolved} (-> (store/ledger db) first :basis)))
      (is (nil? (store/load-screen-of db "unit-4")) "no clearance written"))))
...
(deftest issue-load-test-certificate-always-escalates-then-human-decides
  (testing "a clean, fully-verified, resolved-defect unit still ALWAYS interrupts for human approval -- actuation/issue-load-test-certificate is never auto"
    (let [[db actor] (fresh)
          _ (verify! actor "t8pre" "unit-1")
          _ (screen! actor "t8pre2" "unit-1")
          r1 (exec-op actor "t8" {:op :actuation/issue-load-test-certificate :subject "unit-1"} operator)]
      (is (= :interrupted (:status r1)) "pauses for human approval even when governor-clean")
      (testing "approve -> commit, certificate record drafted"
        (let [r2 (approve! actor "t8")]
          (is (= :commit (get-in r2 [:state :disposition])))
          (is (true? (:load-test-certified? (store/unit db "unit-1"))))
...
(deftest issue-load-test-certificate-double-issuance-is-held
  (testing "issuing the same unit's load-test certificate twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (verify! actor "t10pre" "unit-1")
          _ (screen! actor "t10pre2" "unit-1")
          _ (exec-op actor "t10a" {:op :actuation/issue-load-test-certificate :subject "unit-1"} operator)
          _ (approve! actor "t10a")
          res (exec-op actor "t10" {:op :actuation/issue-load-test-certificate :subject "unit-1"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
