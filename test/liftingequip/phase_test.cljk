(ns liftingequip.phase-test
  "The phase table as executable tests. The invariant this repo cannot
  regress on: `:actuation/dispatch-unit`/`:actuation/issue-load-
...
(deftest issue-load-test-certificate-never-auto-at-any-phase
  (testing "structural invariant: no phase auto-commits a real load-test certificate"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :actuation/issue-load-test-certificate))
          (str "phase " n " must not auto-commit :actuation/issue-load-test-certificate")))))

(deftest load-test-screen-never-auto-at-any-phase
  (testing "screening carries no direct capital risk, but is still never auto-eligible, matching every sibling screening op in this fleet"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :load-test/screen))
          (str "phase " n " must not auto-commit :load-test/screen")))))
...
(deftest gate-escalates-a-clean-non-auto-write
  ...
  (is (= :escalate (:disposition (phase/gate 3 {:op :actuation/issue-load-test-certificate} :commit)))))
