(ns liftingequip.store
  "SSoT for the lifting-equipment-manufacturing actor, behind a
  ...
  (test/liftingequip/store_contract_test.clj), which is the whole
  point: the actor, the Lifting Equipment Governor and the audit
  ...
  load-test certificate) acting on the SAME entity (a unit), each
  with its OWN history collection, sequence counter and dedicated
  double-actuation-guard boolean (`:unit-dispatched?`/
  `:load-test-certified?`, never a `:status` value) -- the same
  ...
  screened for an unresolved proof-load-test defect, which unit
  action was dispatched, which load-test certificate was issued, on
  what jurisdictional basis, approved by whom' is always a query over
  an immutable log -- the audit trail a community trusting a
  lifting-equipment manufacturer needs, and the evidence a
  manufacturer needs if a dispatch or load-test-certificate decision
  is later disputed."
...
(defprotocol Store
  (unit [s id])
  (all-units [s])
  (load-screen-of [s unit-id] "committed proof-load-test screening verdict for a unit, or nil")
  (requirements-verification-of [s unit-id] "committed design-rules requirements verification, or nil")
  (ledger [s])
  (dispatch-history [s] "the append-only unit-dispatch history (liftingequip.registry drafts)")
  (evidence-history [s] "the append-only load-test-certificate history (liftingequip.registry drafts)")
  ...
  (unit-already-certified? [s unit-id] "has this unit's load-test certificate already been issued?")
...
(defn demo-data
  ...
  lifecycles (dispatching a unit action, issuing a load-test
  certificate) so the actor + tests run offline."
  []
  {:units
   {"unit-1" {:id "unit-1" :unit-name "Sakura 10t Overhead Bridge Crane OC-04"
                  :test-load-actual 11.0 :test-load-min 10.0 :test-load-max 12.5
                  :load-test-defect-unresolved? false
                  :unit-dispatched? false :load-test-certified? false
                  :jurisdiction "JPN" :status :intake}
    "unit-2" {:id "unit-2" :unit-name "Atlantis 12t Tower Crane TC-09"
                  :test-load-actual 11.0 :test-load-min 10.0 :test-load-max 12.5
                  :load-test-defect-unresolved? false
                  :unit-dispatched? false :load-test-certified? false
                  :jurisdiction "ATL" :status :intake}
    "unit-3" {:id "unit-3" :unit-name "鈴木産業製ワイヤーロープホイスト WH-07"
                  :test-load-actual 15.0 :test-load-min 10.0 :test-load-max 12.5
                  :load-test-defect-unresolved? false
                  :unit-dispatched? false :load-test-certified? false
                  :jurisdiction "JPN" :status :intake}
    "unit-4" {:id "unit-4" :unit-name "田中カウンターバランスフォークリフト FL-03"
                  :test-load-actual 11.0 :test-load-min 10.0 :test-load-max 12.5
                  :load-test-defect-unresolved? true
                  :unit-dispatched? false :load-test-certified? false
                  :jurisdiction "JPN" :status :intake}}})
...
(defn- issue-load-test-certificate!
  "Backend-agnostic `:unit/mark-certified` -- looks up the
  unit via the protocol and drafts the load-test-certificate
  record, and returns {:result .. :unit-patch ..} for the caller
  to persist."
  [s unit-id]
  (let [a (unit s unit-id)
        seq-n (next-evidence-sequence s (:jurisdiction a))
        result (registry/register-load-test-certificate unit-id (:jurisdiction a) seq-n)]
    {:result result
     :unit-patch {:load-test-certified? true
...
(defrecord MemStore [a]
  ...
  (load-screen-of [_ id] (get-in @a [:load-screens id]))
  ...
  (unit-already-certified? [_ unit-id] (boolean (get-in @a [:units unit-id :load-test-certified?])))
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :load-test-screen/set
      (swap! a assoc-in [:load-screens (first path)] payload)
      ...
      :unit/mark-certified
      (let [unit-id (first path)
            {:keys [result unit-patch]} (issue-load-test-certificate! s unit-id)
            jurisdiction (:jurisdiction (unit s unit-id))]
        (swap! a (fn [state]
                   (-> state
...
(defn seed-db
  ...
  (->MemStore (atom (assoc (demo-data)
                           :verifications {} :load-screens {} :ledger [] :dispatch-sequences {}
...
(def ^:private schema
  ...
  Map/compound values (verification/load-screen payloads, ledger
  ...
  {:unit/id                            {:db/unique :db.unique/identity}
   ...
   :load-screen/unit-id                {:db/unique :db.unique/identity}
...
(defn- unit->tx [{:keys [id unit-name test-load-actual test-load-min test-load-max
  ...
  (cond-> {:unit/id id}
    ...
    test-load-actual                            (assoc :unit/test-load-actual test-load-actual)
    test-load-min                               (assoc :unit/test-load-min test-load-min)
    test-load-max                               (assoc :unit/test-load-max test-load-max)
    (some? load-test-defect-unresolved?)         (assoc :unit/load-test-defect-unresolved? load-test-defect-unresolved?)
    (some? unit-dispatched?)                     (assoc :unit/unit-dispatched? unit-dispatched?)
    (some? load-test-certified?)                 (assoc :unit/load-test-certified? load-test-certified?)
...
(def ^:private unit-pull
  [:unit/id :unit/unit-name :unit/test-load-actual
   :unit/test-load-min :unit/test-load-max
   :unit/load-test-defect-unresolved? :unit/unit-dispatched? :unit/load-test-certified?
   :unit/jurisdiction :unit/status :unit/dispatch-number :unit/evidence-number])

(defn- pull->unit [m]
  (when (:unit/id m)
    {:id (:unit/id m) :unit-name (:unit/unit-name m)
     :test-load-actual (:unit/test-load-actual m)
     :test-load-min (:unit/test-load-min m)
     :test-load-max (:unit/test-load-max m)
     :load-test-defect-unresolved? (boolean (:unit/load-test-defect-unresolved? m))
     :unit-dispatched? (boolean (:unit/unit-dispatched? m))
     :load-test-certified? (boolean (:unit/load-test-certified? m))
...
(defrecord DatomicStore [conn]
  ...
  (load-screen-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?aid
                :where [?k :load-screen/unit-id ?aid] [?k :load-screen/payload ?p]]
              (d/db conn) id)))
  ...
  (unit-already-certified? [s unit-id]
    (boolean (:load-test-certified? (unit s unit-id))))
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      :unit/upsert
      ...
      :load-test-screen/set
      (d/transact! conn [{:load-screen/unit-id (first path) :load-screen/payload (enc payload)}])
