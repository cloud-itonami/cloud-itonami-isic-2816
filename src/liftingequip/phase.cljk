(ns liftingequip.phase
  "Phase 0->3 staged rollout -- the lifting-equipment-manufacturer
  analog of `cloud-itonami-isic-6512`'s `casualty.phase`.
    ...
    Phase 3  supervised auto  -- governor-clean, high-confidence
                                 ...
                                 load-test-certificate` NEVER
                                 auto-commit, at any phase.

  `:actuation/dispatch-unit`/`:actuation/issue-load-test-
  ...
  shipment action on a crane/hoist/forklift unit and issuing a real
  ASME B30/LOLER proof-load-test acceptance certificate are the two
  real-world legal acts this actor performs; both are always a human
  lifting-equipment manufacturing engineer's call.
  `liftingequip.governor`'s `:actuation/dispatch-unit`/`:actuation/
  issue-load-test-certificate` high-stakes gate enforces the same
  invariant independently -- two layers, not one, agree on this.
  `:load-test/screen` is likewise never auto-eligible, at any phase --
  the same posture every sibling's screening op has. Phase 3's
  `:auto` set here has only ONE member (`:unit/intake`) -- this
  domain has no separate no-capital-risk 'file' lifecycle distinct
  from the unit record itself.")

(def read-ops  #{})
(def write-ops #{:unit/intake :design-rules/verify :load-test/screen
                 :actuation/dispatch-unit :actuation/issue-load-test-certificate})

;; NOTE the invariant: `:actuation/dispatch-unit`/`:actuation/
;; issue-load-test-certificate` are members of `write-ops`
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                          :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:unit/intake :design-rules/verify :load-test/screen}          :auto #{}}
...
(defn verdict->disposition
  "Map a Lifting Equipment Governor verdict to a base
