(ns liftingequip.operation
  "OperationActor -- one lifting-equipment-manufacturing operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (Lifting Equipment Advisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Lifting
  Equipment Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one lifting-equipment-manufacturing operation
  ...
  decision to a human operator (the lifting-equipment manufacturing
  engineer). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:actuation/dispatch-unit`/`:actuation/issue-
  load-test-certificate` ALWAYS reach this node when the governor
  is clean -- see `liftingequip.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [liftingequip.liftingequipadvisor :as liftingequipadvisor]
...
(defn build
  ...
  `liftingequip.store/Store`).
  opts:
    :advisor      -- a `liftingequip.liftingequipadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (liftingequipadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Lifting Equipment Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (liftingequipadvisor/-advise advisor store request)]
            {:proposal p :audit [(liftingequipadvisor/trace request p)]})))

      ;; Lifting Equipment Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
